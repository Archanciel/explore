...
targetAudioFileName = sourceName + "_moviepy_22050.mp3"
targetAudioFilePath = targetAudioDir + DIR_SEP + targetAudioFileName
audio.write_audiofile(targetAudioFilePath)
audio.close()

print('\nmoviepy.editor.AudioFileClip({}, fps=22050)\n'.format(sourceVideoFilePath))
...
targetAudioFileName = sourceName + "_ffmpeg_64_44100.mp3"
targetAudioFilePath = targetAudioDir + DIR_SEP + targetAudioFileName
ts = time.time()

# fps=44100 ---> 34.5 sec
ffmpeg_extract_audio(sourceVideoFilePath, targetAudioFilePath, bitrate=64, fps=44100)
print('\nffmpeg_extract_audio({}, {}, bitrate=64, fps=44100)\n'.format(sourceVideoFilePath, targetAudioFilePath))
audioExtractionTime = str(time.time() - ts)
print(audioExtractionTime) # 37.5 sec
newAudioFilePath = targetAudioFilePath[:-4] + '_' + audioExtractionTime + '.mp3'
os.rename(targetAudioFilePath, newAudioFilePath)
...
targetAudioFileName = sourceName + "_ffmpeg_128_22050.mp3"
targetAudioFilePath = targetAudioDir + DIR_SEP + targetAudioFileName
ts = time.time()

# fps=22050 ---> 19.7 sec quality ok !
ffmpeg_extract_audio(sourceVideoFilePath, targetAudioFilePath, bitrate=128, fps=22050)
print('\nffmpeg_extract_audio({}, {}, bitrate=128, fps=22050)\n'.format(sourceVideoFilePath, targetAudioFilePath))
audioExtractionTime = str(time.time() - ts)
print(audioExtractionTime) # 37.5 sec
newAudioFilePath = targetAudioFilePath[:-4] + '_' + audioExtractionTime + '.mp3'
os.rename(targetAudioFilePath, newAudioFilePath)
