...
class SelectableLabel(RecycleDataViewBehavior, Label):
	...
	def apply_selection(self, rv, index, is_selected):
		...
		if not self.selected and not is_selected:
			...
			print(info.f_code.co_name, info.f_lineno, 'index ', index, 'self.selected ', self.selected, 'is_selected ', is_selected, '--> return', 'sel nodes', rv.children[0].selected_nodes)
			return
		elif self.selected and not is_selected:
			# toggling from selected to unselected
			print(info.f_code.co_name, info.f_lineno, 'index ', index, 'self.selected ', self.selected, 'is_selected ', is_selected, '--> self.selected = False', 'sel nodes', rv.children[0].selected_nodes)
			if rv.children[0].selected_nodes:
				rv.children[0].selected_nodes[0] = ''
			self.selected = False                   #<<------
		else:
			# toggling from unselected to selected
			print(info.f_code.co_name, info.f_lineno, 'index ', index, 'self.selected ', self.selected, 'is_selected ', is_selected, '--> self.selected = not self.selected', 'sel nodes', rv.children[0].selected_nodes)
